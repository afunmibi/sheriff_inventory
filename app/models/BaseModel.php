...
/**
 * Base Model Class
 * Abstract class for all models with CRUD operations
 */
...
abstract class BaseModel {
    ...
    public function getAll(array $columns = ['*'], array $options = []): array {
        ...
        $query = DatabaseConnection::table($this->table)->select($select);
    ...
    public function findWhere(array $conditions): ?array {
        $query = DatabaseConnection::table($this->table);
    ...
    public function create(array $data): int {
        ...
        $id = DatabaseConnection::table($this->table)->insert($filteredData);
    ...
    protected function logAudit(string $action, int $entityId, ?array $oldData, ?array $newData): void {
        try {
            ...
            DatabaseConnection::table('audit_logs')->insert([
                'user_id' => $userId,
                'action' => $action,
    ...
    public static function __callStatic(string $method, array $args) {
        ...
        if (str_starts_with($method, 'findBy')) {
            ...
            return DatabaseConnection::table($instance->table)
                ->where($column, $args[0])
                ->first();
        }
